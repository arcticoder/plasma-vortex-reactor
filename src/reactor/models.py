...
def microwave_maxwell(
    E: np.ndarray, sigma: float, eps_r: float, mu_r: float, k0: float
) -> np.ndarray:
    """Shape-preserving attenuation proportional to sigma: E_next = E / (1 + alpha),
    alpha ~ sigma/eps_r.
    """
...
def kinetics_update(
    N: np.ndarray, dt: float, rates: Optional[Dict[str, float]] = None
) -> np.ndarray:
    """Non-stiff kinetics step with optional linear decay k and source S:
    N_next = N + dt*(S - k*N).
    """
